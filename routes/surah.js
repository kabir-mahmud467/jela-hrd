...
router.get('/', async (req,res,next)=>{
  try{
    ...
    const items=await Surah.find(buildFilter(q, phase)).sort({order:1,createdAt:1}).limit(100).lean();
...
router.get('/porbo/:phase', async (req,res,next)=>{
  try{
    ...
    const items=await Surah.find(buildFilter('', phase)).sort({order:1,createdAt:1}).limit(100).lean();
    res.render('surah', { items, q:'', phase, phases: Question.PHASES });
  }catch(e){ next(e); }
});
