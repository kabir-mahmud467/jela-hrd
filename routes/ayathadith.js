...
router.get('/', async (req, res, next) => {
  try {
    ...
    const items = await AyatHadith.find(buildFilter(q, phase, kind)).sort({ order: 1, createdAt: 1 }).limit(200).lean();
...
router.get('/porbo/:phase', async (req, res, next) => {
  try {
    ...
    const kind = (req.query.kind || '').toString().slice(0, 20);
    const items = await AyatHadith.find(buildFilter('', phase, kind)).sort({ order: 1, createdAt: 1 }).limit(200).lean();
